...
function getWorldSpaceDimensionsAtZ(z = camera.position.z) {
  const vFOV = THREE.MathUtils.degToRad(camera.fov);
  const visibleHeight = 2 * Math.tan(vFOV / 2) * z;
  const visibleWidth = visibleHeight * camera.aspect;
  return { width: visibleWidth, height: visibleHeight };
}
...
function getBoundingRadius(obj) {
  ...
  return sphere.radius; // in world units at current object scale
}

function offscreenX(radius, dir = 1, margin = 0.12) {
  const { width } = getWorldSpaceDimensionsAtZ(camera.position.z);
  const halfW = width / 2;
  return dir * (halfW + radius + width * margin);
}


function offscreenY(radius, dir = 1, margin = 0.12) {
  const { height } = getWorldSpaceDimensionsAtZ(camera.position.z);
  const halfH = height / 2;
  return dir * (halfH + radius + height * margin);
}
</script>
