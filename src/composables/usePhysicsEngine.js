...
export function useElementTracker(containerRef, options = {}) {
  ...
  const gatherElements = (parentEl, containerRect, relative, excludeClasses) => {
    const result = []
    
    const processElement = (el) => {
      if (excludeClasses.some(className => el.classList.contains(className))) {
        return
      }
      ...
      if (['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'span', 'div'].includes(data.tagName)) {
        ...
        data.textX = relative ? textPos.x - containerRect.left : textPos.x;
        data.textY = relative ? textPos.y - containerRect.top : textPos.y;
      }
  ...
  const updateElementData = () => {
    ...
    elementData.value = {
      ...
      elements: gatherElements(container, containerRect, relative, excludeClasses),
      timestamp: now
    }
  }
